...
func (s *NodeService) AddFavorite(userID uint, nodeID string) error {
	return s.nodeRepo.AddFavorite(userID, nodeID)
}

func (s *NodeService) RemoveFavorite(userID uint, nodeID string) error {
	return s.nodeRepo.RemoveFavorite(userID, nodeID)
}

func (s *NodeService) GetFavorites(userID uint) ([]model.Favorite, error) {
	return s.nodeRepo.GetFavorites(userID)
}

func (s *NodeService) GetRegions() ([]string, error) {
	return s.nodeRepo.GetRegions()
}
